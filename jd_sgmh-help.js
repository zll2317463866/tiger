...
!(async () => {
  ...
  console.log(`格式化后的助力码::${JSON.stringify(inviteCodes)}\n`);
                ...
                if (!code) continue
                await harmony_collectScore(code, 3);
                await $.wait(2000)
              }
  }
        ...
        try {
          ...
          for (let i = 0;i < data.data.result.taskVos.length;i ++) {
            ...
            if (data.data.result.taskVos[i].taskName === '邀请好友助力') {
              ...
              inviteCodes.push(data.data.result.taskVos[i].assistTaskDetailVo.taskToken)
              $.log('收集邀请码！！！')

...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      if ($.canHelp) {
        console.log(`开始账号内部互助 赚京豆-瓜分京豆 活动，如有剩余则给作者tiger助力`)
        for (let j = 0; j < $.authorTuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给作者tiger ${$.authorTuanList[j]['assistedPinEncrypted']}助力`)

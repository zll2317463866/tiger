...
// 要助力的cookie编号
// 需要助力的cookie id
let doCookie = [1,2,3,4];
// 跳过的ck，比如前20个已经跑了，就填20，运行就从21开始
let start = 0
// cookie 位置
let cookiesArr =[];
// ==========================
...
!(async () => {
  ...
  console.log('检测账号：')
  tuanIds=[]
  for await (const id of doCookie) {
   cookie = cookiesArr[id - 1];
    await tuanActivity();
  }
  console.log('本次执行助力码：')
  tuanIds.map(code => console.log(code))
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              tuanIds.push(userTuanInfo.tuanId)
            } else {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data['ret'] === 10005 || data['ret'] === 10206){
                //火爆，或者今日参团机会已耗尽
                $.canHelp = false;
              }
              if (data['ret'] === 10209||data['ret'] === 10216){
                //已成团 或 团已结束
                tuanIds = tuanIds.filter(t=>t!==tuanId);
              }
            }
          }
        }
...
function requireConfig() {
  return new Promise(async resolve => {
    tuanActiveId = process.env.TUAN_ACTIVEID || tuanActiveId;
    if (!tuanActiveId) {
      console.log(`无自定义拼团活动ID`)
    } else {
    ...
    $.shareCodesArr = [];
    shareCodes.map(code=> $.shareCodesArr.push(code))

...
async function run(type = 0){
  try{
    rebateCodes = ["Sdu19Mo","SwfDVb6"];
    rebateCodes = rebateCodes[Math.floor((Math.random() * rebateCodes.length))]
    rebateCodes = $.isNode() ? (process.env.rebateCode ? process.env.rebateCode : `${rebateCodes}`) : ($.getdata('jd_rebateCode') ? $.getdata('jd_rebateCode') : `${rebateCodes}`);

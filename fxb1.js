const $ = new Env('鎼炲熀澶х-鍙戣储鎸栧疂help');
      ...
      if (!$.isLogin) {
        ...
        continue
          ...
          if (process.env.fcwbinviteCode && process.env.fcwbinviteCode.indexOf('@') > -1) {
            ...
            console.log(`閭€璇风爜鎮ㄩ€夋嫨鐨勬槸鐢�"@"闅斿紑\n`)
        } else {
        ...
        if (fcwbinviters[item]) {
          ...
          console.log(`鍏�${fcwbinviteCodeArr.length}涓個璇风爜`)
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
             if(data.success==true){
               console.log(`鎸栧埌${data.data.chunk.value}`)  
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
             if(data.success==true){
               console.log('浠诲姟锛�'+data.errMsg)  
             }else if(data.success==false){
             console.log('浠诲姟宸茬粡瀹屾垚')}
          }
        }
      } catch (e) {
        $.logErr(e, resp)
